...
@interface CosjiNewWebViewController ()

@end

@implementation CosjiNewWebViewController
@synthesize customNavBar,storeName,webView;
static CosjiNewWebViewController *shareCosjiNewWebViewController = nil;
+(CosjiNewWebViewController*)shareCosjiNewWebViewController
{
    
    if (shareCosjiNewWebViewController == nil) {
        shareCosjiNewWebViewController = [[super allocWithZone:NULL] init];
    }
    return shareCosjiNewWebViewController;
}
...
- (void)viewDidLoad
{
    ...
    self.storeName.text=[NSString stringWithFormat:@"%@",self.itemName];
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.urlStirng]]];
}
