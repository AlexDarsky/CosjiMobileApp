...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [specialActivityViewController.tabBarItem setFinishedSelectedImage:[UIImage imageNamed:@"新折扣优惠-动态"] withFinishedUnselectedImage:[UIImage imageNamed:@"新折扣优惠-默认"]];
   ...
   [self initCustomTabBarView];
    if ([[NSUserDefaults standardUserDefaults] floatForKey:@"APPversion"]==NSNotFound||[[NSUserDefaults standardUserDefaults] floatForKey:@"APPversion"]<1.0)
    {
        [[NSUserDefaults standardUserDefaults] setFloat:1.0 forKey:@"APPversion"];
    }
...
-(void)initCustomTabBarView
{
    ...
    NSArray *itemsArray=[NSArray arrayWithObjects:@"新首页",@"新淘宝返利",@"新折扣优惠",@"新我的可及", nil];
    for (int x=0; x<[itemsArray count]; x++)
    {
        UIButton *indexButton=[UIButton buttonWithType:UIButtonTypeCustom];
        indexButton.frame=CGRectMake(152/2*x, -5, 152/2, 90/2);
