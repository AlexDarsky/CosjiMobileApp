...
/*
 ...
 7 额度大于5
 */
@interface CosjiGetCashJifenbaoViewController ()
{
...
@implementation CosjiGetCashJifenbaoViewController
...
- (void)loadView
{
    ...
    [getCashBtn setTitle:@"集分宝提现" forState:UIControlStateNormal];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    avilibaleCashLabel.text=[NSString stringWithFormat:@"%@",[infoDic objectForKey:@"jifenbao"]];
...
-(void)goTixian
{
    if ([requestCashField.text isEqualToString:@""]||requestCashField.text==nil) {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"错误" message:@"请输入提现" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil];
        [alert show];
        return;
    }
    if ([requestCashField.text floatValue]>[avilibaleCashLabel.text floatValue])
    {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"错误" message:@"提现集分宝大于可提现集分宝数量" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil];
        [alert show];
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        CosjiServerHelper *serverHelper=[CosjiServerHelper shareCosjiServerHelper];
        NSDictionary *responeDIC=[serverHelper getJsonDictionary:[NSString stringWithFormat:@"/account/apply/?type=1&&money=%@&&code=%@&&realname=%@&&mobile=%@&&password=%@",requestCashField.text,zhifubaoIDField.text,payeeNameField.text,phoneNumberField.text,passwordField.text]];
        if (responeDIC!=nil) {
